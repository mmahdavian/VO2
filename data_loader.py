...
class Feeder:
    ...
    def preprocess(self):
        ...
        df.drop(['VCO2','RR','VE','ID_y','ID_x','Humidity','Temperature'], axis=1, inplace=True) 
        df_cleaned = df
        ...
        for id_value, group in groups.items():
            ...
            for i in range(0, len(df_numerical) - (self.past_data + self.future_data) * self.time_interval, self.interval):
                ...
                temporal_sequence_data.append(combined_sequence)
                ## non temporal data
                general_data.append([cur_df_imputed[i,:4]])
                output.append(self.target_variable[i+ self.past_data * self.time_interval])
        ...
        self.input_general_data = np.array(self.input_general_data).squeeze(1).astype(np.float32)
